...
public class JDBCDataSetGenerator implements DataSetGenerator {
	...
	Logger logger = LoggerFactory.getLogger(JDBCDataSetGenerator.class);
	...
	public DataSet generate(String sql) {
		...
		Arrays.stream(metaData.getColumnNames()).map(c -> new StringCell(c)).forEach(c -> colNames.add(c));
		...
		while (dbRow.next()) {
			...
			for (int i = 1; i <= metaData.getColumnCount(); i++) {
				...
				case Types.TIMESTAMP:
					...
					break;
				default:
					temp = new StringCell(dbRow.getObject(i).toString());
				}
				row.add(temp);
			}
			data.add(row);
